...
class OrdersPendingController extends GetxController {
  ...
  List<OrdersModel> filteredData(String orderType) {
    if (orderType.contains(",")) {
      // For multiple status types (like "1,2,3")
      List<String> statuses = orderType.split(",");
      return data.where((order) => statuses.contains(order.ordersStatus)).toList();
    } else {
      // For single status type
      return data.where((order) => order.ordersStatus == orderType).toList();
    }
  }
