...
import 'items_seller/add/add.dart';
...
class ProfilePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    _scrollController.addListener(() {
      if (_scrollController.position.pixels >= _scrollController.position.maxScrollExtent - 100) {
        if (itemsController.hasMore && !itemsController.isLoadingMore) {
            ...
            child: Column(
              ...
              children: [
                ...
                if (userProfile?.usersPhone != null &&
                  ...
                  Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          userProfile.usersPhone!.startsWith('964')
                              ? '0${userProfile.usersPhone!.substring(3)}'
                ...
                if (myServices.sharedPreferences.getString("id") != null &&
                    myServices.sharedPreferences.getString("id")!.isNotEmpty)
                  Row(
                    children: [
                      Icon(Icons.person, size: 16, color: Colors.grey), // تغيير الأيقونة من phone إلى person
                      SizedBox(width: 4),
                      Expanded(
                        child: Text(
                          "المعرف : ${myServices.sharedPreferences.getString("id")!}", // إضافة "ID: " قبل الرقم
                          style: Theme.of(context).textTheme.bodyMedium,
                          overflow: TextOverflow.ellipsis,
                        ),
                      ),
                        ...
                        children: [
                          Expanded(
                            child: _buildProductCard(
                              itemsController.data[firstIndex],
                              firstIndex,
                            ),
                          ),
                          ...
                          if (secondIndex < itemsController.data.length)
                            Expanded(
                              child: _buildProductCard(
                                itemsController.data[secondIndex],
                                secondIndex,
                              ),
                            )
                          else
  ...
  Widget _buildLoader(ItemsControllerSeller controller) {
    ...
    if (!controller.hasMore && controller.data.isNotEmpty) {
      return Container(
        padding: const EdgeInsets.symmetric(vertical: 20),
        child: Center(
          child: Text(
                        ...
                        children: [
                          ...
                          Builder(
                            builder: (context) {
                              ...
                              return Text(
                                isOutOfStock ? "نفذ المخزون".tr : "${"متوفر: ".tr}${itemsController.data[index].itemsCount}${" قطعة".tr}",
                                style: TextStyle(
                                  fontSize: 11,
                          ...
                          Column(
                            ...
                            children: [
                              if (hasDiscount) ...[
                                Text(
                                  "${itemsController.data[index].itemsPriceDiscount} ${"د.ع".tr}",
                                  style: const TextStyle(
                                    fontSize: 14,
                                ...
                                Text(
                                  "${itemsController.data[index].itemsPrice} ${"د.ع".tr}",
                                  style: TextStyle(
                                    fontSize: 11,
                                    color: Colors.grey[500],
